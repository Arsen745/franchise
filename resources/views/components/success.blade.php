<section class="success desktop">
    <div class="container">
        <h1 class="title">ОТКРЫВАЕМ УСПЕШНЫЙ <br>БИЗНЕС ЗА 60 ДНЕЙ </h1>
    </div>
    <div class="block1">
        <div class="lines">
            <img class="line" src="{{ asset('assets/line1.png') }}" alt="">
            <div class="arrow container">
                <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                <img class="right" src="{{ asset('assets/right.png') }}" alt="">
            </div>
            <div class="number container">
                <h4>1</h4>
                <h4>2</h4>
                <h4>3</h4>
                <h4>4</h4>
                <h4>5</h4>
                <h4>6</h4>
            </div>
        </div>
        <div class="success-cards container">
            <div class="success-card">
                <h3>Знакомство и первая встреча</h3>
                <p>5 дней</p>
            </div>
            <div class="success-card">
                <h3>Подписание NDA и изучение документов</h3>
                <p>3 дня</p>
            </div>
            <div class="success-card">
                <h3>Утверждение партнёра</h3>
                <p>5 дней</p>
            </div>
            <div class="success-card">
                <h3>Подписание основного договора</h3>
                <p>3 дня</p>
            </div>
            <div class="success-card">
                <h3>Поиск и согласование локации</h3>
                <p>4-10 дней</p>
            </div>
            <div class="success-card">
                <h3>Выезд франчайзи в головной офис и старт обучения</h3>
            </div>
        </div>
    </div>
    <div class="block1">
        <div class="lines">
            <img class="line" src="{{ asset('assets/line1.png') }}" alt="">
            <div class="arrow container">
                <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                <img class="right" src="{{ asset('assets/right.png') }}" alt="">
            </div>
            <div class="number number2 container">
                <h4>7</h4>
                <h4>8</h4>
                <h4>9</h4>
                <h4>10</h4>
                <h4>11</h4>
                <img src="{{ asset('assets/success-image.png') }}" alt="">
            </div>
        </div>
        <div class="success-cards container">
            <div class="success-card">
                <h3>Приобретение материалов и оборудования</h3>
            </div>
            <div class="success-card">
                <h3>строительно-монтажные работы</h3>
                <p>30 дней</p>
            </div>
            <div class="success-card">
                <h3>Подбор и обучение персонала</h3>
            </div>
            <div class="success-card">
                <h3>Оформление первой заявки</h3>
                <p>3 дня</p>
            </div>
            <div class="success-card">
                <h3>Выезд startup команды</h3>
                <p>7 дней до открытия <br>7 дней после открытия</p>
            </div>
            <div class="success-card">
                <h3>Открытие фирменного магазина</h3>
            </div>
        </div>
    </div>
</section>
{{--section for mobile slider--}}
<section class="success mobile">
    <h1 class="title">ОТКРЫВАЕМ УСПЕШНЫЙ БИЗНЕС ЗА 60 ДНЕЙ </h1>
    <div class="block swiper mySwiperSuccess">
        <div class="cards swiper-wrapper">
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>Знакомство <br>и первая <br>встреча</h3>
                </div>
                <div class="number-arrow">
                    <h4>1</h4>
                    <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                </div>
                <div class="date-success">
                    <p>5 дней</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>Подписание <br>NDA и изучение <br>документов</h3>
                </div>
                <div class="number-arrow">
                    <h4>2</h4>
                    <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                </div>
                <div class="date-success">
                    <p>3 дня</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>Утверждение <br>партнёра</h3>
                </div>
                <div class="number-arrow">
                    <h4>3</h4>
                    <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                </div>
                <div class="date-success">
                    <p>5 дней</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>Подписание <br>основного <br>договора</h3>
                </div>
                <div class="number-arrow">
                    <h4>4</h4>
                    <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                </div>
                <div class="date-success">
                    <p>3 дня</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>Поиск и <br>согласование <br>локации</h3>
                </div>
                <div class="number-arrow">
                    <h4>5</h4>
                    <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                </div>
                <div class="date-success">
                    <p>4-10 дней</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>Выезд <br>франчайзи в <br>головной офис и <br>старт обучения</h3>
                </div>
                <div class="number-arrow">
                    <h4>6</h4>
                    <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                </div>
                <div class="date-success">
                    <p class="none">_</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>Приобретение <br>материалов <br>и оборудования</h3>
                </div>
                <div class="number-arrow">
                    <h4>7</h4>
                    <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                </div>
                <div class="date-success">
                    <p class="none">_</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>строительно- <br>монтажные <br>работы</h3>
                </div>
                <div class="number-arrow">
                    <h4>8</h4>
                    <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                </div>
                <div class="date-success">
                    <p>30 дней</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>Подбор <br>и обучение <br>персонала</h3>
                </div>
                <div class="number-arrow">
                    <h4>9</h4>
                    <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                </div>
                <div class="date-success">
                    <p class="none">_</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>Оформление <br>первой заявки</h3>
                </div>
                <div class="number-arrow">
                    <h4>10</h4>
                    <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                </div>
                <div class="date-success">
                    <p>3 дня</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>Выезд startup <br>команды</h3>
                </div>
                <div class="number-arrow">
                    <h4>11</h4>
                    <img class="right" src="{{ asset('assets/right.png') }}" alt="">
                </div>
                <div class="date-success large">
                    <p>7 дней до открытия <br>7 дней после открытия</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
            <div class="card-success swiper-slide">
                <div class="title-card-success">
                    <h3>Открытие <br>фирменного <br>магазина</h3>
                </div>
                <div class="number-arrow shop-image">
                    <img class="shop" src="{{ asset('assets/success-image.png') }}" alt="">
                </div>
                <div class="date-success">
                    <p class="none">_</p>
                </div>
                <div class="line">
                    <img src="{{ asset('assets/line1.png') }}" alt="">
                </div>
            </div>
        </div>
    </div>
    <div class="button-success">
        <img src="{{ asset('assets/button.png') }}" alt="">
    </div>
</section>
@push('scripts')
    @Vite(['resources/css/components/success.css'])
    @Vite(['resources/js/components/success.js'])
@endpush
